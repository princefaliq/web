...
@section('content')
    ...
    <script type="text/javascript">
        ...
        {
            //$(#created).validate();
            jQuery('select[name="shift"]').on('change',function(){
                var data_shift = jQuery(this).val();
                var url = '/create_press/getnik1/'+data_shift;
                ...
                if(data_shift)
                {
                    jQuery.ajax({
                        ...
                        {
                            console.log(data);
                            jQuery('select[name="nik1"]').empty();
                            $('select[name="nik1"]').append('<option value="">Pilih NIK</option>');
                            jQuery.each(data,function (key,value) {
                                $('select[name="nik1"]').append('<option value="'+ key + '">'+ key +'-' + value + '</option>');
                            })
                        }
                    })
                ...
                {
                    $('select[name="nik1"]').empty();
                }
            });

            jQuery('select[name="nik1"]').on('change',function(){
                var nik1 = jQuery('select[name="nik1"]').val();
                var data_shift = jQuery('select[name="shift"]').val();

                    jQuery.ajax({
                        url : "{{ route('getnik2prees') }}?data_shift=" + data_shift + "&nik=" + nik1,
                        type : 'GET',
                        dataType : 'json',
                        success : function (data)
                        {
            ...
            jQuery('select[name="nik2"]').on('change',function(){
                var nik1 = jQuery('select[name="nik1"]').val();
                var nik2 = jQuery('select[name="nik2"]').val();
                var data_shift = jQuery('select[name="shift"]').val();

                jQuery.ajax({
                    url : "{{ route('getnik3prees') }}?data_shift=" + data_shift + "&nik1=" + nik1 + "&nik2=" + nik2,
                    type : 'GET',
                    dataType : 'json',
                    success : function (data)
                    {
            ...
            jQuery('select[name="nik3"]').on('change',function(){
                var nik1 = jQuery('select[name="nik1"]').val();
                var nik2 = jQuery('select[name="nik2"]').val();
                var nik3 = jQuery('select[name="nik3"]').val();
                var data_shift = jQuery('select[name="shift"]').val();

                jQuery.ajax({
                    url : "{{ route('getnik4prees') }}?data_shift=" + data_shift + "&nik1=" + nik1 + "&nik2=" + nik2 + "&nik3=" + nik3,
                    type : 'GET',
                    dataType : 'json',
                    success : function (data)
                    {
